...
var contractAddress = '0x8CB77815221d76B014058594b8634cB01893232D'; // FIXME: fill this in with your contract's address/hash
...
async function getTotalOwed(user) {
	return await BlockchainSplitwise.methods.getTotalOwed(user).call();
}
...
async function getLastActive(user) {
	return await BlockchainSplitwise.methods.getLastActive(user).call();
}
...
async function add_IOU(creditor, amount) {
	...
	if (path !== null) {
		...
		for (var i = 0; i < path.length - 1; i++) {
			await BlockchainSplitwise.methods.addIOU(path[i + 1], - min, Math.floor(new Date().getTime()/1000)).send({ from: path[i], gas: 3000000 });
		}

	}
	return await BlockchainSplitwise.methods.addIOU(creditor, amount - min, Math.floor(new Date().getTime()/1000)).send({ from: web3.eth.defaultAccount, gas: 3000000 });
}

async function lookup(debtor, creditor) {
	return await BlockchainSplitwise.methods.lookup(debtor, creditor).call();
}

async function getNeighbors(user) {
	return await BlockchainSplitwise.methods.getNeighbors(user).call();
}
